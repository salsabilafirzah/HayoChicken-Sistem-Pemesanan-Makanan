...
class _SearchScreenState extends ConsumerState<SearchScreen> {
  ...
  void initState() {
    super.initState();
    // Auto-focus immediately when screen pushes
  ...
  Widget build(BuildContext context) {
    ...
    final filteredProducts = query.isEmpty 
        ? [] 
        : state.products.where((p) => p.name.toLowerCase().contains(query)).toList();
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            "Cari Menu Favoritmu",
            style: GoogleFonts.inter(fontSize: 20, fontWeight: FontWeight.w800, color: Colors.black54),
          ),
          const SizedBox(height: 8),
          Text(
            "Ketik nama menu di atas",
...
class _SearchWaveClipper extends CustomClipper<Path> {
  @override
